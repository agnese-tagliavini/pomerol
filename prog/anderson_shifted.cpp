...
int ceil_div2( int W_index )
{
   return ( W_index - 1000000 ) / 2 + 500000; 
}

int floor_div2( int W_index )
{
   return ( W_index + 1000000 ) / 2 - 500000; 
}

template <typename T>
ComplexType chi_pp_shift_f(T const& chi, int W_index, int nu_index, int nup_index, double beta) { 
   int w1_index = nu_index + ceil_div2( W_index ); 
   int w2_index = floor_div2( W_index ) - nu_index - 1; 
   int w3_index = floor_div2( W_index ) - nup_index - 1; 
   return chi(I*FMatsubara( w1_index, beta ), I*FMatsubara( w2_index, beta ), I*FMatsubara( w3_index, beta )); // this comes from Pomerol - see TwoParticleGF::operator()
}

template <typename T>
ComplexType chi_ph_shift_f(T const& chi, int W_index, int nu_index, int nup_index, double beta) { 
   int w1_index = nu_index - floor_div2( W_index ); 
   int w2_index = nup_index + ceil_div2( W_index ); 
   int w3_index = nup_index - floor_div2( W_index ); 
   return chi(I*FMatsubara( w1_index, beta ), I*FMatsubara( w2_index, beta ), I*FMatsubara( w3_index, beta )); // this comes from Pomerol - see TwoParticleGF::operator()
}

template <typename T>
ComplexType chi_xph_shift_f(T const& chi, int W_index, int nu_index, int nup_index, double beta) { 
   int w1_index = nu_index - floor_div2( W_index ); 
   int w2_index = nup_index + ceil_div2( W_index ); 
   int w3_index = nu_index + ceil_div2( W_index ); 
   return chi(I*FMatsubara( w1_index, beta ), I*FMatsubara( w2_index, beta ), I*FMatsubara( w3_index, beta )); // this comes from Pomerol - see TwoParticleGF::operator()
}

//template <typename T>
//ComplexType chi_ferm(T const& chi, int w1_index, int w2_index, int w1p_index, double beta) { 
//todo
   //int w3_index = w1_index + w2_index - w1p_index;  // w3 entspricht w2p
   //return chi(I*FMatsubara( w1_index, beta ), I*FMatsubara( w2_index, beta ), I*FMatsubara( w3_index, beta )); // this comes from Pomerol - see TwoParticleGF::operator()
//}
...
int main(int argc, char* argv[])
{
   ...
   bool notation_pp, notation_ph, notation_xph, notation_ferm; 
   size_t L;
   ...
   try { // command line parser
      ...
      //TCLAP::SwitchArg twopgf_notation_pp("","Fpp","Use particle-particle notation (shifted) for 2-particle Green's function",notation_pp, false);
      //TCLAP::SwitchArg twopgf_notation_ph("","Fph","Use particle-hole notation (shifted) for 2-particle Green's function",notation_ph, false);
      //TCLAP::SwitchArg twopgf_notation_xph("","Fxph","Use exchange-particle-hole notation (shifted) for 2-particle Green's function",notation_xph, false);
      //TCLAP::SwitchArg twopgf_notation_ferm("","Fferm","Use fermionic notation for 2-particle Green's function",notation_ferm, false);
   ...
   if (calc_2pgf) {   
      ...
      indices4.insert(IndexCombination4(u0,d0,d0,u0)); // register up-down-down-up component for evaluation
      //indices4.insert(IndexCombination4(u0,d0,u0,d0)); // register up-down-down-up component for evaluation 
      ...
      Chi4.KroneckerSymbolTolerance = 1e-16;
	 ...
	 { // dispatch and save two-particle GF data - MPI parallelization in bosonic freqs
	    ...
	    for (pMPI::MPIWorker worker(comm,ROOT);!worker.is_finished();) {
		  ...
		  double W_index = job_to_bfreq_index(p, wb_max); 

		  std::cout << "["<<p+1<<"/" << ntasks << "] p" << comm.rank() << " Omega_idx = " << W_index << std::endl;

		  std::ofstream chi_pp_shift_stream(("chi_pp_shift_"+ind_str+"_W"+boost::lexical_cast<std::string>(p)+".dat").c_str());
		  std::ofstream chi_ph_shift_stream (("chi_ph_shift_"+ind_str+"_W"+boost::lexical_cast<std::string>(p)+".dat").c_str());
		  std::ofstream chi_xph_shift_stream (("chi_xph_shift_"+ind_str+"_W"+boost::lexical_cast<std::string>(p)+".dat").c_str());
		  //std::ofstream chi_ferm_stream (("chi_ferm_shift_"+ind_str+"_W"+boost::lexical_cast<std::string>(p)+".dat").c_str());

		  // Most important part 2 - loop over fermionic frequencies. Consider parallelizing one of the loop
		  for (int nu_index = -wf_max; nu_index<wf_max; nu_index++) { // loop over first fermionic frequency 
		     for (int nup_index = -wf_max; nup_index<wf_max; nup_index++) { // loop over second fermionic

			ComplexType val_pp_shift = chi_pp_shift_f(chi, W_index, nu_index, nup_index, beta);
			ComplexType val_ph_shift = chi_ph_shift_f(chi, W_index, nu_index, nup_index, beta);
			ComplexType val_xph_shift = chi_xph_shift_f(chi, W_index, nu_index, nup_index, beta);
                        //ComplexType val_ferm = chi_ferm(chi, W_index, nu_index, nup_index, beta); // Note that W=w1 nu=w2 nup=w1p

			//chi_pp_shift_stream << std::scientific << std::setprecision(12)  << W_index << " "
			   //<< nu_index << " " << nup_index << "   " << std::real(val_pp_shift) << " " << std::imag(val_pp_shift) << std::endl;

			chi_pp_shift_stream << std::scientific << std::setprecision(12) << W_index << " " << nu_index << " " << nup_index << "   " << std::real(val_pp_shift) << " " << std::imag(val_pp_shift) << std::endl;
			chi_ph_shift_stream << std::scientific << std::setprecision(12)  <<  W_index << " " << nu_index << " " << nup_index << "   " << std::real(val_ph_shift) << " " << std::imag(val_ph_shift) << std::endl;
			chi_xph_shift_stream << std::scientific << std::setprecision(12)  << W_index << " " << nu_index << " " << nup_index << "   " << std::real(val_xph_shift) << " " << std::imag(val_xph_shift) << std::endl;
			//chi_ferm_stream << W_index << " " << nu_index << " " << nup_index << "   " << std::real(val_ferm_shift) << " " << std::imag(val_ferm_shift) << std::endl;
		     }
		     //chi_stream << std::endl;
		  };
		  chi_pp_shift_stream.close();
		  chi_ph_shift_stream.close();
		  chi_xph_shift_stream.close();
